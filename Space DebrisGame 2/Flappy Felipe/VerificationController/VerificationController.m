...
void* base64_decode(const char* s, size_t* data_len_ptr)
{
    ...
    if (len % 4)
        [NSException raise:@"Invalid input in base64_decode" format:@"%zu is an invalid length for an input string for BASE64 decoding", len];
...
- (void)setUp:(id)parent :(SEL)selector {
    //NSLog(@"set up");
   // handler = parent;
   // action = selector;
}
...
- (void)verifyPurchase:(SKPaymentTransaction *)transaction completionHandler:(VerifyCompletionHandler)completionHandler
{
    ...
    NSString *payload = [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\", \"password\" : \"%@\"}",
       ...
       requestData = [payload dataUsingEncoding:NSUTF8StringEncoding];
...
//        BOOL isOk = [self isTransactionAndItsReceiptValid:transaction];
    NSArray *inAppDictionary = [[jsonResponse objectForKey:@"receipt"] objectForKey:@"in_app"];

              int i=0;
              for (NSDictionary *dict in inAppDictionary) {
                  i++;
                  if (i==inAppDictionary.count) {
                      
                      if ([[dict valueForKey:@"product_id"] isEqualToString:@"com.globussoft.SDPnewLife"]) {
                          
            BOOL transactionValid = [self isTransactionAndItsReceiptValid:transaction purchaseDict:dict];
                          if (transactionValid) {
...
{
    /*
    NSLog(@"isTransactionAndItsReceiptValid  %@",transaction.description);
    NSLog(@"isTransactionAndItsReceiptValid  %@",transaction);
  ...
  if (!(transaction && transaction.transactionReceipt && [transaction.transactionReceipt length] > 0))
   ...
   NSDictionary *receiptDict  =[NSJSONSerialization JSONObjectWithData:requestData options:0 error:&error];
    
//    NSDictionary *receiptDict  = [self dictionaryFromPlistData:requestData];
...
//    }
    ...
    if (![self doTransactionDetailsMatchPurchaseInfo:transaction withPurchaseInfo:dict])
    {
        return NO;
    }
    ...
    [transactionsReceiptStorageDictionary setObject:dict forKey:transactionId];
...
//    =============================
    ...
    if (!(transaction && transaction.transactionReceipt && [transaction.transactionReceipt length] > 0))
    {
        ...
        //NSLog(@"isTransactionAndItsReceiptValid further");
        // Pull the purchase-info out of the transaction receipt, decode it, and save it for later so
        // it can be cross checked with the verifyReceipt.
...
{/*
    ...
    NSLog(@"transaction product identifier quantity tarnsctionid %@  %ld  %@",transaction.payment.productIdentifier,(long)transaction.payment.quantity,transaction.transactionIdentifier);
    
    if (![transaction.payment.productIdentifier isEqualToString:[purchaseInfoDict objectForKey:@"product_id"]])
    {
    ...
    if (![transaction.transactionIdentifier isEqualToString:[purchaseInfoDict objectForKey:@"transaction_id"]])
    {
        failCount++;
    }
...
-(void) savePurchaseDetails:(NSDictionary *)dict{
    
    NSString * transaction_id  =[[dict objectForKey:@"receipt"]objectForKey:@"transaction_id"];
        //    NSString * purchase_date =[dict objectForKey:@"purchase_date"];
    NSString *uduid = [[[UIDevice currentDevice] identifierForVendor] UUIDString];
    
    PFObject *object = [PFObject objectWithClassName:AppPurchaseTableName];
    object[@"DeviceUniqueID"] = uduid;
    object[@"DeviceType"] =@"iOS";
    object[@"TransactionID"] = transaction_id;
        //    object[@"PurchaseDate"] = purchase_date;
        // object[@"count"] = 5;
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    NSDictionary *verifiedReceiptDictionary = [self dictionaryFromJSONData:[receiptString dataUsingEncoding:NSUTF8StringEncoding]];
        ...
        //NSLog(@"dict %@", verifiedReceiptDictionary);
          ...
          isEqualToString:[purchaseInfoFromTransaction objectForKey:@"bid"]])
    {
        ...
        //NSLog(@"step 4 passed %i",failCount);
          ...
          isEqualToString:[purchaseInfoFromTransaction objectForKey:@"product-id"]])
    {
        ...
        //NSLog(@"step 5 passed %i",failCount);
          ...
          isEqualToString:[purchaseInfoFromTransaction objectForKey:@"quantity"]])
    {
        ...
        //NSLog(@"step 6 passed %i",failCount);
          ...
          isEqualToString:[purchaseInfoFromTransaction objectForKey:@"item-id"]])
    {
        ...
        //NSLog(@"step 7 passed %i",failCount);
    ...
    if ([[UIDevice currentDevice] respondsToSelector:NSSelectorFromString(@"identifierForVendor")]) // iOS 6?
    {
        ...
        if(verifiedReceiptVendorIdentifier)
        {
                ...
                || ![purchaseInfoUniqueVendorId isEqualToString:localIdentifier])
            {
                ...
                //NSLog(@"step 8 iOS6 passed %i",failCount);
        ...
        //NSLog(@"step 9 passed %i",failCount);
...
{
    NSString *responseString = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    //NSLog(@"connection didReceiveData: %@",responseString);
    NSError *error = nil;
    id jsonObject1 = [NSJSONSerialization
    ...
    if (jsonObject1 != nil && error == nil)
    {
        if ([jsonObject1 isKindOfClass:[NSDictionary class]])
        {
            ...
            deserializedDictionary = (NSDictionary *)jsonObject1;
        }
    }
    ...
    if (isOk)
    {
        ...
        NSString *strProductId = [[deserializedDictionary objectForKey:@"receipt"] objectForKey:@"product_id"];
        
        if ([strProductId isEqualToString:@"com.globussoft.SDPnewLife"]) {
            
            NSInteger life=[[NSUserDefaults standardUserDefaults]integerForKey:@"life"];
            [[NSUserDefaults standardUserDefaults] setInteger:500+life forKey:@"life"];
            [self savePurchaseDetails:deserializedDictionary];
        }
